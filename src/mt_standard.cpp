...
int main(int argc, char * argv[])
{
  ...
  while (!exiter.exit()) {
    ...
    if (mode.load() == io::Mode::auto_aim) {
      ...
      Eigen::Quaterniond q;
      try { q = gimbal.imu_at(t - 1ms); } catch (const io::AhrsSampleUnavailable &) { continue; }
      ...
      commandgener.push(targets, t,  gimbal.bullet_speed_value(), ypr);  // 发送给决策线程
    ...
    /// 打符
      ...
      if (mode.load() == io::Mode::small_buff) {
        ...
        buff_command = buff_aimer.aim(target_copy, t,  gimbal.bullet_speed_value(), true);
