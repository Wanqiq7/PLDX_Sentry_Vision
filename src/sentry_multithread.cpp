...
int main(int argc, char * argv[])
{
  ...
  while (!exiter.exit()) {
    ...
    Eigen::Quaterniond q;
    try { q = gimbal.imu_at(timestamp - 1ms); } catch (const io::AhrsSampleUnavailable &) { continue; }
    ...
    auto directive = ros2.subscribe_target_directive();
    decider.apply_target_directive(armors, directive);

    decider.set_priority(armors);

    auto detection_queue = perceptron.get_detection_queue();

    for (auto & detection : detection_queue) {
      decider.apply_target_directive(detection.armors, directive);
    }
    ...
    ros2.publish(target_info, timestamp);
  }
