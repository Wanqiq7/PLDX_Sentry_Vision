...
io::Command Decider::decide(
  ...
  io::USBCamera & usbcam2, io::Camera & back_camera, const io::TargetDirective & directive)
{
  ...
  apply_target_directive(armors, directive);
  empty = armors.empty();
...
io::Command Decider::decide(
  ...
  const io::TargetDirective & directive)
{
  cv::Mat img;
  std::chrono::steady_clock::time_point timestamp;
...
void Decider::sort(std::vector<DetectionResult> & detection_queue)
{
  ...
  remove_empty_detections(detection_queue);
...
void Decider::remove_empty_detections(std::vector<DetectionResult> & detection_queue)
{
  detection_queue.erase(
    std::remove_if(
      detection_queue.begin(), detection_queue.end(),
      [](const DetectionResult & detection) { return detection.armors.empty(); }),
    detection_queue.end());
}
  ...
  const Eigen::Matrix3d & R_gimbal2world)
{
  ...
  for (const auto & armor : armors) {
    if (armor.name == target.name) {
      const auto & state = target.ekf().x;
      const Eigen::Vector3d center_in_world{state[0], state[2], state[4]};
      const Eigen::Vector3d center_in_gimbal = R_gimbal2world.transpose() * center_in_world;
      return Eigen::Vector4d{
        center_in_gimbal[0], center_in_gimbal[1], center_in_gimbal[2],
        static_cast<double>(armor.name) + 1};  // avoid ambiguity; IDs are 1-based
    }
  }
...
void Decider::apply_target_directive(
  std::list<auto_aim::Armor> & armors, const io::TargetDirective & directive)
{
  if (!directive.valid) return;

  const auto contains_id = [](const std::vector<int8_t> & ids, const auto_aim::Armor & armor) {
    const auto id = static_cast<int8_t>(armor.name + 1);
    return std::find(ids.begin(), ids.end(), id) != ids.end();
  };

  armors.remove_if(
    [&](const auto_aim::Armor & armor) {
      return contains_id(directive.excluded_target_ids, armor);
    });

  if (directive.restrict_to_preferred_targets) {
    armors.remove_if(
      [&](const auto_aim::Armor & armor) {
        return !contains_id(directive.preferred_target_ids, armor);
      });
  }
}
